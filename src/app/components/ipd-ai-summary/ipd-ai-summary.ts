...
Use the following exact JSON schema:
{
  ...
  "principal_diagnosis": "Text description of the condition established after study to be chiefly responsible for the admission.",
  "icd_10_pdx": "The exact ICD-10 2016 code for the principal diagnosis.",
  "secondary_diagnoses": [
    {
      "description": "Text description of the secondary diagnosis or comorbidity.",
      "icd_10_code": "The exact ICD-10 2016 code.",
      "is_cc_mcc": "Boolean: true if this condition acts as a CC or MCC for DRG weighting, false otherwise."
    }
  ],
  "procedures": [
    {
      "description": "Text description of the surgical or medical procedure performed.",
      "icd_9_cm_code": "The exact ICD-9-CM 2015 code."
    }
  ],
  "drg_code": "The calculated DRG code (e.g., '01931').",
  "drg_description": "The official description of the assigned DRG.",
  "drg_rationale": "Brief clinical rationale for the DRG assignment, noting how the principal diagnosis, procedures, and CC/MCC influenced the final DRG."
}

If there are no secondary diagnoses or procedures, use an empty array [] for those keys. Do not use the string "None".
    `;
    let payloadEnglish = '';
    if (data?.sex) {
    ...
    this.aiPrompt.set({ system: systemMessageEnglish1, user: this.aiContent });
    const messages = [
      {
        role: "system",
        content: systemMessageEnglish1
      },
      {
        role: "user",
        content: this.aiContent
        // content: aiUserContents // example
      }
    ];
    const result: any = await this.aiService.aiCompletions(messages);
    console.log('AI Completions result:', result);
    if (result?.status != 200) {
      this.isLoading.set(false);
      return this.toastr.error(result?.status + ' ' + result?.message || '', 'AI API Error', { progress: true });
    }
